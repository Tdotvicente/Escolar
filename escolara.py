...
Script: menu_cli.py
Descrição:
Este script implementa um menu de interface de linha de comando (CLI) para a manipulação de dados de alunos e professores. 
Ele encapsula as operações CRUD (Create, Read, Update, Delete) e herda funcionalidades dos scripts de manipulação de banco de dados.

Funcionalidades principais:
1. Limpa o excesso de informação na tela e deixando o visual menos poluido
2. Herda métodos dos scripts Bd_Alunos e Bd_Professores.
3. Permite interagir com os bancos de dados para cadastrar, ler, atualizar, excluir e buscar registros de alunos e professores.
4. Oferece um menu amigável para navegação em um ambiente CLI.
5. Cada submenu é responsável por operações específicas no banco de dados.

Observação:
Para o correto funcionamento deste script, é necessário que os scripts Bd_Alunos, Bd_Professores e Limpar_tela estejam implementados 
e acessíveis no mesmo diretório ou caminho do ambiente Python.
"""

from crude_aluno import Bd_Alunos
from crude_professor import Bd_Professores
import os
...
class Menu_cli(Bd_Alunos, Bd_Professores):
    ...
    def submenu_ler(self):
        ...
        if submenu.lower() == 'a':
            ...
            alunos = self.bd_alunos.ler_dados_aluno(nome_parcial)
            ...
            professores = self.bd_professores.ler_dados_professor(nome_parcial)

            if professores:
                print("Professor(es) encontrado(s)!\n")
    ...
    def submenu_atualizar(self):
        ...
        if submenu .lower() == 'a':
            ...
            alunos = self.bd_alunos.ler_dados_aluno(info_aluno)
            ...
            if alunos:
                ...
                self.bd_alunos.alterar_dados_aluno(info_aluno, novo_dado)
            ...
            professores = self.bd_professores.ler_dados_professor(info_professor)
    ...
    def submenu_buscar(self):
        ...
        if submenu.lower() == 'a':
            ...
            resultado = self.bd_professores.buscar_dados(nome_professor)

            if resultado:
                print("Professor(es) encontro(s)!")
